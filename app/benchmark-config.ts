...
export const BENCHMARK_STAGES: Array<{ stage: BenchmarkStage; name: string; description: string }> = [
  ...
  { stage: 1, name: "+ PRESSURE", description: "하강 웨이브와 CORE LINE 추가" },
  { stage: 2, name: "+ ITEMS", description: "경험치와 드롭 아이템 추가" },
  { stage: 3, name: "+ BRICK TYPES", description: "가드·실드 브릭 추가" },
  { stage: 4, name: "+ SKILLS", description: "레벨업과 패들 스킬 추가" },
  { stage: 5, name: "+ BOSSES", description: "20웨이브 보스와 보상 추가" },
];
...
export function normalizeBenchmarkConfig(saved: unknown): BenchmarkConfig {
  ...
  const targetWave = Number(source.targetWave);
  return {
    ...
    targetWave: targetWave === 40 || targetWave === 60 || targetWave === 100 ? targetWave : 20,
  };
}
